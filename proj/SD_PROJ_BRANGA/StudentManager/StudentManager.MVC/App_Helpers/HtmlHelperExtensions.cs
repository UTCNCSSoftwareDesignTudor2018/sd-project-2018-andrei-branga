...
// pentru aside pentru a face active daca dai click pe el(mere in jos submeniul)
namespace Restaurant.MVC.App_Helpers
{
  public static class HtmlHelperExtensions
  {
    ...
    public static IHtmlString RouteIf(this HtmlHelper helper, string value, string attribute)
    {
      ...
      var controllerActions = value.Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries);
      var result = false;
      foreach (var val in controllerActions)
      {
        var valSplit = val.Split(new[] {'/'}, StringSplitOptions.RemoveEmptyEntries);
        var controller = valSplit.Length > 0 ? valSplit[0] : string.Empty;
        var action = valSplit.Length > 1 ? valSplit[1] : currentAction;
        ...
        result |= hasController && hasAction;
        if (result)
          break;
      }

      return result ? new HtmlString(attribute) : new HtmlString(string.Empty);
    }
